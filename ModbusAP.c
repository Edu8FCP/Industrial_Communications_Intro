...
int Read_h_regs(char * server_addr, int port, uint32_t st_r, uint16_t n_r, uint16_t * val){
    ...
    if(NULL == server_addr){
        ...
        val = NULL;
        return -4;
    } // Não tenho a certeza se faz sentido mas, ...
    /* else if(n-r > (253 - 1 (func) - 2 (start addr) - 2 (nr of regs))/2 )
    NÃO SEI SE FAZ SENTIDO, SE SE PODE PEDIR MAIS QUE ESTE NUMERO MAS VIR EM VÁRIAS FRAMES */ 
    /* else if ((uint32_t)n_r > (253-1-2-2)/2){
        printf("[ERRO x - adicionado depois] Read_h_regs - ModbusAP Layer don't support so many values \n");
        val = NULL;
        return -4;
    } */
    ...
    // normal response
    // In an exception response, the server
    // returns an exception code in the data field. This defines the server condition that caused the
    // exception.

    if(APDU_func + 0x80 == APDU_R[0]){  // hadling the error existence in the previous apdu
        switch(APDU_R[1]){
            ...
            case 0x04:                  // device failure
                printf("[ERROR SERVER_DEVICE_FAILURE] Read_h_regs\n");
                val = NULL;
                free(APDU);
                APDU = NULL;
                free(APDU_R);
    ...
    // returns: number of read regs – ok, <0 – error
...
int Write_multiple_regs(char * server_addr, int port, uint32_t st_r, uint16_t n_r, uint16_t * val){
    ...
    // 2 para o endereço inicial, 2 para o nr de registos, 1 para o código da função e um para o campo len (byte count)
    ...
    // 1 byte function code, 2 bytes starting address e 2 bytes quantity of registers
